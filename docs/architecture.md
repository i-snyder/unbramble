...
- `meta_kv.index_complete` — crash/fatal-error guard around every mutating pull or push pass. A pass writes `0` before its first mutation and `1` only after inventory, serialized references, and C# analysis all finish. A missing key means complete for databases created by older binaries, but only a heartbeat with the current freshness-protocol stamp may vouch for it. Queries never trust a heartbeat while the marker is `0`; recovery contends directly on the finite `IndexWriterLock` and performs a clean full rebuild, independent of the lifetime `WatcherLock` an idle watcher continues to hold.
...
- **Trust heartbeats only when their schema stamp, independent freshness-protocol stamp, and `meta_kv.index_complete` marker all match the current contract.** A live watcher with an incompatible stamp must be retired with `unbramble stop`; sweeping alongside it would leave the old process free to publish an unguarded partial batch afterward. An incomplete current-protocol index waits for the active writer or performs a clean full rebuild after taking the writer lease.
