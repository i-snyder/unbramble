...
- `.meta` identity extraction is itself bounded line streaming and stops as soon as the identity `guid:` is found; a huge importer payload before the identity can't force a whole-file allocation.

### Parser

- Single-pass, bounded line streaming: ordinary lines are materialized up to 1 MiB. Unity's enormous embedded texture/mesh scalars are consumed without materializing their values only after the reader proves every value character is hexadecimal; the parser still receives the indentation/key surrogate so line numbers, sequence indexes, and property paths remain exact. Any other oversized line fails the index instead of risking OOM or silently discarding possible references. Every serialized-reference line parser and meta identity scan uses the same hard materialization bound.
- A `.asset` whose initial bytes contain binary control bytes, or invalid UTF-8 without a recognizable Unity YAML header, is treated as binary content: skip its content pass, but retain its file node, identity GUID, and importer references from the sibling `.meta`. Recognizable YAML with malformed encoding or control bytes fails loudly instead of silently losing edges. UTF-8/UTF-16/UTF-32 BOMs are recognized before binary classification, and the full bounded reader uses strict decoding so invalid bytes after the sniff window fail the same way.
- Reference readers allow concurrent readers and atomic delete/replace saves, but deny concurrent in-place writes so one parse can't observe a hybrid file version. A replacement's watcher event schedules the new version normally.
...
- `meta_kv.index_complete` — crash/fatal-error guard around every mutating pull or push pass. New and rebuilt stores start at `0`; a pass writes `1` only after inventory, serialized references, and C# analysis all finish. Queries never trust a heartbeat while the marker is `0`; recovery contends directly on the finite `IndexWriterLock` and performs a clean full rebuild, independent of the lifetime `WatcherLock` an idle watcher continues to hold. Transient stat/open failures propagate instead of being published as missing identity or empty references. The watcher suppresses freshness as soon as the first event is accepted and resumes only after a successful commit with no later paths pending; failed batches requeue without creating a freshness window.
- `meta_kv.store_instance_id` — random durable identity regenerated whenever the cache is created or schema-rebuilt. A heartbeat must name this exact database instance, preventing a fresh file left beside a deleted or redirected cache from vouching for unrelated contents. Current-schema opens validate both required v11 metadata keys; schema creation publishes `schema_version` last so an interrupted metadata transition is rebuilt on the next open rather than accepted as current.
...
Store rules: recursive walks use a closure CTE followed by a non-recursive `MIN(depth)` display query and a depth cap. Diff batches delete before inserting, distinguish moves from live GUID collisions, and run transactionally. Every foreign-key child key used by a delete action must have an index; in particular, `symbol_refs.source_symbol_id` needs one so incremental symbol replacement doesn't scan the entire reference table once per deleted symbol. Install `busy_timeout` before lock-taking pragmas; current-schema opens are read-only after any missing access paths are repaired under the writer lease. Every finite mutation owns exclusive writer intent plus `index-writer.lock`, separate from the watcher's lifetime lock. NativeAOT requires `SQLitePCL.Batteries_V2.Init()`.
...
- **Trust heartbeats only when their schema stamp, independent freshness-protocol stamp, database instance id, active watcher-session id, watcher freshness byte, and `meta_kv.index_complete` marker all match the current contract.** A live watcher with an incompatible claim must be retired with `unbramble stop`; sweeping alongside it would leave that process free to publish into the wrong contract afterward. An incomplete current-protocol index waits for the active writer or performs a clean full rebuild after taking the writer lease. A query must acquire the shared-open legacy guard before any recovery or sweep, so a `FileShare.None` watcher can never overlap a mutation by a newer binary.
- **Hold a query read lease after freshness succeeds.** `IndexReadLock` is the shared side of the same cross-process file-share contract as `IndexWriterLock`: multiple queries can coexist, but a writer can't start until every query engine is disposed. A separate writer-intent gate stops admitting new readers once a writer queues, preventing continuous query traffic from starving watcher updates. The marker is rechecked after acquiring the lease, closing the gap between `EnsureFresh` and the actual resolve/graph/liveness reads without forcing one long SQLite transaction or disabling query-time cache publication. `stats` is the deliberate exception: as a non-waiting status command it reports the last committed snapshot without taking a query lease.
- **Single active watcher, enforced:** current watchers hold an OS byte-range owner lock in `.unbramble/watcher.lock` and record a random owner session in the file tail. A second byte-range transition gate makes owner acquisition, graceful disposal, session publication, and inherited-heartbeat invalidation one stable snapshot to queries; snapshots re-probe ownership after reading the session and freshness state because process death releases locks without taking that gate. A third byte is held throughout promotion and whenever accepted or resync work is pending; queries reject the heartbeat while that byte is locked, even if another process denies heartbeat-file deletion. Promotion, targeted batches, and full resyncs share one operation gate, and a resync registers its intent before waiting on that gate, so queued operations can't resume each other's freshness claim. A failed full resync leaves a required-resync latch set, preventing any later targeted batch from publishing freshness until a complete sweep succeeds. Each query also retains a shared-open legacy guard from freshness evaluation through its graph reads, preventing an older `FileShare.None` watcher from starting in the boundary after the snapshot while remaining compatible with current byte-range watchers. Redundant workers exit successfully; the OS releases either lock form on process death. On startup: start watchers buffering *first*, then catch-up sweep, then drain — sweeping before watching leaves an unwatched gap that the heartbeat then vouches for.
...
Type selection for a bound method comes **only** from `m_TargetAssemblyTypeName`  — never from `m_Target`'s guid, which identifies the containing asset, not a script. Cascade: exact single member match in a semantic-mode assembly → `proven`; multiple overloads → `advisory` link to every overload (never guess which one Unity picked); no declared match → walk the project's `inherit` chain upward, any match found this way is `advisory`; no type resolvable or no match anywhere → an unmatched-advisory edge carrying the raw name, and the name enters the liveness screen set. Guid-less same-asset bindings (the common case — button and handler in one prefab) have no resolvable target chain at all; they contribute a `name_hints` row only, and rely on the handler's own `m_Script` attachment for file-level liveness. Member lookup retains the exact/escaped-prefix predicate but bounds it through `idx_symbols_docid`; one resolution pass memoizes repeated type/member/inheritance lookups, and its complete result is reusable only while `build_reachable_state.graph_generation` is unchanged. These are access-path optimizations over the same cascade, not persisted or approximate links. The snapshot is local to one store instance: separate one-shot CLI processes validate the same persisted generation but don't share the derived rows in memory.
...
- **Indexing and query performance** — no-change sweeps and asset queries meet their targets. Roslyn scheduling still lacks a width/memory cap at very high assembly counts; persisted compilation artifacts and USN-journal sweeps aren't yet justified. Separate concurrent semantic-query processes can repeat UnityEvent resolution and other derived reads, then contend for CPU and the filesystem cache; SQLite WAL keeps the answers independent and correct, but latency can rise compared with the same queries run sequentially. Don't persist or share those derived results without the same generation validation used by the in-process cache.
...
- **Static discovery only.** A fresh coding-agent session otherwise has no way to learn UnBramble exists, so `init` upserts an idempotent, version-stamped, marker-delimited block (`<!-- unbramble:begin/end -->`) into project-root `AGENTS.md`. The block makes UnBramble the default for relationship discovery, explicitly separates caller/reference enumeration from legitimate literal text search, and carries an independent guidance revision. The zero-argument home command reports an old revision and points to `init`; it never edits the file itself.
...
- Performance targets (design scale ~100k files): full index < 2 min, no-change refresh < 2s, and indexed asset queries < 100ms. Semantic queries may take low single-digit seconds because they resolve conservative cross-domain evidence at query time; they must complete predictably without stale answers or silent stalls. Targets, not license to trade away correctness — see the "Indexing and query performance" bullet in Known gaps above for what's still open.
