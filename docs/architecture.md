...
- A `.asset` whose initial bytes contain binary control bytes or invalid UTF-8 is treated as binary content: skip its content pass, but retain its file node, identity GUID, and importer references from the sibling `.meta`. UTF-8/UTF-16/UTF-32 BOMs are recognized before binary classification.
...
- **Trust heartbeats only when their schema stamp matches `CurrentSchemaVersion` and `meta_kv.index_complete` is true.** A schema mismatch triggers an inline sweep and tells the user to retire the old watcher with `unbramble stop`; an incomplete index waits for the active writer or performs a clean full rebuild after taking the writer lease.
